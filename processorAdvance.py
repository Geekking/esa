...
#tempfilepath ='/home/lanny/Desktop/test.xml'
#生成的SQL文档所在目录绝对路径
#运行前需要修改
tempoutputfiledirpath = "/home/lanny/Documents/project/wiki_xml_doc/test/outSQL/"


categoryPattern = re.compile(u'\[\[Category:[:_a-zA-Z0-9\|\-\*\(\) ]*?\]\]')
disambiguationPattern = re.compile('.*?(D|d)isambiguation pages.*?')
disambiguationPattern2 = re.compile('\{\{(.*?(D|d)isambiguation.*?)|(.*?(H|h)ndis.*?)|(.*?(G|g)eodis.*?)|(.*?(L|l)ists of ambiguous numbers.*?)\}')
innerLinkPattern = re.compile(u'\[\[[a-zA-Z\|\-\(\)0-9: ]+?\]\]')
...
class XMLProcessor:
    ...
    def OutputFilePreprocess(self):
        ...
        createNamespaceTableSql = '''
             DROP TABLE IF EXISTS `namespace`;\n
                CREATE TABLE `namespace` (`id` int(10) unsigned NOT NULL AUTO_INCREMENT,PRIMARY KEY (`id`)) ENGINE=InnoDB AUTO_INCREMENT=230395 DEFAULT CHARSET=utf8;\n '''
        createArticleTableSql = '''
               DROP TABLE IF EXISTS `article`;\n
                CREATE TABLE `article` (`id` int(10) unsigned NOT NULL AUTO_INCREMENT,`title` varchar(100) DEFAULT NULL,`abstract` varchar(2000) DEFAULT NULL,`anchor` varchar(1000) DEFAULT NULL,`text`  longtext ,`anchor2` longtext ,PRIMARY KEY (`id`),KEY `title` (`title`(15)))ENGINE=InnoDB AUTO_INCREMENT=353354 DEFAULT CHARSET=utf8;\n '''
        createLeafCatTableSql = '''DROP TABLE IF EXISTS `leafCat`;\n
                CREATE TABLE `leafCat` ( `id` int(10) unsigned NOT NULL AUTO_INCREMENT,`title` varchar(100) DEFAULT NULL,`leafcat` varchar(1000) DEFAULT NULL,PRIMARY KEY (`id`), KEY `title` (`title`(15))) ENGINE=InnoDB AUTO_INCREMENT=332485 DEFAULT CHARSET=utf8;\n'''
        createCategoryTableSql = '''DROP TABLE IF EXISTS `category`;\n
                CREATE TABLE `category` (`id` int(10) unsigned NOT NULL AUTO_INCREMENT,`cat_title` varchar(100) DEFAULT NULL,`p_cat` varchar(1000) DEFAULT NULL,`c_cat` varchar(1000) DEFAULT NULL,PRIMARY KEY (`id`), KEY `title` (`cat_title`(15))  ) ENGINE=InnoDB AUTO_INCREMENT=77953 DEFAULT CHARSET=utf8;\n'''
        createDisambiguateTableSql = '''
                DROP TABLE IF EXISTS `disambiguation`;\n
                CREATE TABLE `disambiguation` (`id` int(10) unsigned NOT NULL AUTO_INCREMENT,`title` varchar(100) DEFAULT NULL,`dis_title` varchar(1000) DEFAULT NULL, PRIMARY KEY (`id`),KEY `title` (`title`(15)))ENGINE=InnoDB AUTO_INCREMENT=230395 DEFAULT CHARSET=utf8;\n '''
        createPagelinksTableSql= '''
                DROP TABLE IF EXISTS `pagelinks`;\n
                CREATE TABLE `pagelinks` (`source_id` int(10) unsigned NOT NULL , `target_title` varchar(100) ,PRIMARY KEY (`source_id`))ENGINE=InnoDB AUTO_INCREMENT=230395 DEFAULT CHARSET=utf8;\n '''
        createRedirectTableSql = '''
                DROP TABLE IF EXISTS `redirect`;\n
                CREATE TABLE `redirect` ( `id` int(10) unsigned NOT NULL AUTO_INCREMENT,`title` varchar(100) DEFAULT NULL, `rd_title` varchar(100) DEFAULT NULL, PRIMARY KEY (`id`), KEY `title` (`title`(15)) )ENGINE=InnoDB AUTO_INCREMENT=230395 DEFAULT CHARSET=utf8;\n '''
    ...
    def processText(self,elem,textSubelem,page_id,catFlag= False):
        ...
        global partionPattern,skipPattern,removePattern,removePattern2
        ...
        if textSubelem.text != None:
            ...
            for eachline in last:
                ...
                if match:
                    disFlag = True
                    oneCat = re.sub(quotePattern,'\'\'',match.group(0)[2:])
